...
\paragraph{Definition Dimension}
...
\paragraph{Spans and Independence}
...
\subparagraph{Implications of Sums and Spans}
If \(X = Y = \mathbb{R}\) then obviously \(X \oplus Y = \mathbb{R}\).
Also, \( 
    \spans\{\vv\} \oplus \spans\{\vw\} = \spans\{\vv, \vw\}.
\)

\paragraph{Uniqueness of Direct Sums}
The sum of subspace \(S, T\) is direct if and only if any vector
\(\vx \in S + T\) can be written uniquely as \( \vx = \vs + \vt \)
where \(\vs \in S, \vt \in T\).

\paragraph{Dimensions of Sums}
Suppose that \(S\) and \(T\) are of finite dimension.
Then, \[
    \dim(S) + \dim(T) = \dim(S + T) + \dim(S \cap T).
\]

Observe that for direct sums, \(\dim(S \cap T) = 0\).

\paragraph{Complementary Space}
If \(V\) is a finite dimensional vector space and \(X \leq V\)
then, there is a subspace \(Y\)
such that \(X \oplus Y = V\).

We call \(Y\) a complementary space, which is not unique because we
can pick \(Y\) to be any basis of \(V - X\).

\subsection{External Direct Sums}

\paragraph{External Direct Sums}
Let \(X, Y\) be two vector spaces over the same field \(\mathbb{F}\).
Then, the cartesian product \(X \times Y\) can be made into a vector space
over \(\mathbb{F}\) with the obvious definitions
\[
    (\vx_1, \vx_2) + (\vy_1, \vy_2) = (\vx_1 + \vy_1 , \vx_2 + \vy_2)
\]
and
\[
    \lambda (\vx_1, \vx_2) = (\lambda \vx_1 , \lambda \vx_2).
\]
This cartesian product is the \underline{external} direct sum of \(X, Y\).
That is, \(X \oplus Y\).
This is opposed to the \textit{internal} direct sum covered earlier.

\paragraph{External Direct Sums and Dimension}
Obviously, for \(X, Y\) of finite dimension,
\[
    \dim(X \oplus Y) = \dim(X) + \dim(Y).
\]
If \(X, Y\) are subset of some vector space \(V\) then the external direct
sum can still be identified, even if \(X \cup Y\) is non-trivial.

The internal and external sums of subspaces can be identified if and only if,
the subspaces have trivial intersection.
